...
export function Header({
  ...
  onClick={(event) => {
    ...
    if (!isGeneratingOutline && prompt) {
      handleRegenerate();
    }
  }}
  className={cn(
    "h-7 shrink-0 cursor-pointer gap-1.5 rounded-full border px-3 text-xs inline-flex items-center",
    (isGeneratingOutline || !prompt) && "pointer-events-none opacity-50",
  )}
>
  <RefreshCw
    className={cn(
